# Deep-Agent LanGraph 框架编写思路分享

项目基于 Langchain 官方的`deepagents`进行改写，适配 国内的部分顶尖模型，采用 langgraph 构建。
...
当然这个项目编写的目的也只是个人觉得目前 langchain 的 deepagent 架构本身很不错，但是缺少一些比较重要的内容那比如上下文如何隔离，同时还有一个目的就是测试国内的模型的工具调用能力，根据这个项目的运行结果（编写的时候接入的是 qwen 和 deepseek 系列）来看`deepseek-v3.1`和`qwen3-235b-a22b-instruct-2507`都具备较强的工具调用能力，但是`deepseek`模型更强一点，原因是因为它能稳定的进行上十次的工具调用，且每一次工具调用都较为准确，更难能可贵的是，有一次它发现子智能体执行的结果不是很满意，它还是重新调用再次执行。
