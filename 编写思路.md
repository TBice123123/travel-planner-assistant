...
**笔记写入**
这是因为写入内容通常较长且需要从任务执行结果中提取关键信息，系统选用了性价比高且工具调用能力出色的 `qwen-flash` 模型。该模型不仅成本低廉、响应速度快，还具备不错的工具调用能力。当然为了保证每一次都能正确调用工具，这里采用了 tool choice 的方法实现强制工具调用。同时该智能体还会生成一份非常简短的任务摘要返回给主智能体。

## 最后总结

当然这个项目编写的目的也只是个人觉得目前 langchain 的 deepagent 架构本身很不错，但是缺少一些比较重要的内容那比如上下文如何隔离，同时还有一个目的就是测试国内的模型的工具调用能力，根据这个项目的运行结果。

- `deepseek-v3.1`的工具调用能力较强，且能稳定的进行上十次的工具调用，且每一次工具调用都较为准确，更难能可贵的是，有一次它发现子智能体执行的结果不是很满意，它还是重新调用再次执行。
  ![alt text](./assets/image.png)
- `qwen3-235b-a22b-instruct-2507`这个模型，有一定情况下会出现循环工具调用的情况（循环工具调用 tavily_search）总体来说还行。
- `glm4.5`模型，这个模型也有不错的工具调用能力，但是与 v3.1 有差距。

另外针对`qwen3-235b-a22b-instruct-2507`这个模型，有一定情况下会出现循环工具调用的情况（循环工具调用 tavily_search）。故后续采用`glm4.5`模型替代。
